...
std::optional<PlayerRecord> MysqlClient::getPlayer(const std::string& username) {
    ...
    // escape above already locked; unlock before calling escape recursively
    // Actually we're already holding the lock. Let's use a raw query instead.
    // Re-implement without calling escape (which would deadlock)
    std::string escaped(username.size() * 2 + 1, '\0');
    size_t len = mysql_real_escape_string(conn_, escaped.data(), username.c_str(), username.size());
    escaped.resize(len);
    std::string full_sql = "SELECT id, username, password, rating, total_games, wins "
                           "FROM players WHERE username='" + escaped + "'";

    if (mysql_query(conn_, full_sql.c_str()) != 0) {
...
uint64_t MysqlClient::createPlayer(const std::string& username,
    ...
    size_t lu = mysql_real_escape_string(conn_, eu.data(), username.c_str(), username.size());
    size_t lp = mysql_real_escape_string(conn_, ep.data(), password_hash.c_str(), password_hash.size());
    eu.resize(lu); ep.resize(lp);
    std::string sql = "INSERT INTO players (username, password) VALUES ('" + eu + "', '" + ep + "')";
    if (mysql_query(conn_, sql.c_str()) != 0) {
        spdlog::error("createPlayer failed: {}", mysql_error(conn_));
        return 0;
    }
...
void MysqlClient::recordLogin(uint64_t player_id) {
    ...
    if (mysql_query(conn_, sql.c_str()) != 0) {
        spdlog::error("recordLogin failed: {}", mysql_error(conn_));
    }
}
...
void MysqlClient::saveBattleResult(uint64_t player_id, uint64_t room_id,
    ...
    if (mysql_query(conn_, oss.str().c_str()) != 0) {
        spdlog::error("saveBattleResult failed: {}", mysql_error(conn_));
    }
}
